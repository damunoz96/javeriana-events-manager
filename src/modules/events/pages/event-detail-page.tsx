...
export function EventDetailPage({ eventId }: { eventId: string }) {
  ...
  const { data: leadsData, isLoading: leadsLoading, isFetching: leadsFetching } = useQuery(
    eventLeadsQueryOptions(eventId, debouncedLeadsSearch, leadsPage),
  );
  ...
  const startDate = program.start_date
    ? new Date(program.start_date).toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })
    : null;
  const endDate = program.end_date
    ? new Date(program.end_date).toLocaleDateString('es-CO', { day: 'numeric', month: 'long', year: 'numeric' })
    : null;
      ...
      <Tabs defaultValue="leads">
        ...
        <TabsContent value="leads" className="space-y-4">
          ...
          <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
            <div className={`overflow-x-auto ${isLeadsFiltering ? 'opacity-50 transition-opacity' : 'transition-opacity'}`}>
